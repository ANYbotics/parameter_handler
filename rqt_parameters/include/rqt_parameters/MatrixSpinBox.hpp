...
class MatrixSpinBox : public QWidget {
  ...
  static_assert(std::is_base_of<QAbstractSpinBox, SpinBoxType_>::value,
                "[MatrixSpinBox]: QAbstractSpinBox must be a base of SpinBoxType_!");

 public:
  /*** Constructor
   *   @param parent widget
   */
  MatrixSpinBox(QWidget* parent) : QWidget(parent), grid_(this), spinboxes_(), rows_(0), cols_(0) {
    // Setup grid
   ...
   *   @return      Spinbox
   */
  std::unique_ptr<SpinBoxType_>& getSpinbox(std::size_t row, std::size_t col) { return spinboxes_.at(row * cols_ + col); }
