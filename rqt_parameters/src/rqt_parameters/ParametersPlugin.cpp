...
ParametersPlugin::ParametersPlugin()
    : rqt_gui_cpp::Plugin(),
      ...
      widget_(nullptr),
      paramsGrid_(),
      paramsWidget_(nullptr),
      paramsScrollHelperWidget_(nullptr),
      paramsScrollLayout_(nullptr) {
...
void ParametersPlugin::initPlugin(qt_gui_cpp::PluginContext& context) {
  ...
  if (getNodeHandle().hasParam(namespaceParameterName)) {
    ...
    if (!handlerNamespace.empty() && handlerNamespace.at(0) != '/') {
...
void ParametersPlugin::setNamespace() {
  ...
  if (checkNamespace(text)) {
    ...
    shutdownServices();
    getParameterListClient_ =
        getNodeHandle().serviceClient<parameter_handler_msgs::GetParameterList>(text.toStdString() + getParameterListServiceName_);
    getIntegralParameterClient_ =
        getNodeHandle().serviceClient<parameter_handler_msgs::GetIntegralParameter>(text.toStdString() + getIntegralParameterServiceName_);
    setIntegralParameterClient_ =
        getNodeHandle().serviceClient<parameter_handler_msgs::SetIntegralParameter>(text.toStdString() + setIntegralParameterServiceName_);
    getFloatingPointParameterClient_ = getNodeHandle().serviceClient<parameter_handler_msgs::GetFloatingPointParameter>(
        text.toStdString() + getFloatingPointParameterServiceName_);
    setFloatingPointParameterClient_ = getNodeHandle().serviceClient<parameter_handler_msgs::SetFloatingPointParameter>(
        text.toStdString() + setFloatingPointParameterServiceName_);
    refreshAll();
    ROS_INFO("[rqt_parameters]: Set namespace %s ", text.toStdString().c_str());
  }
  auto completer = new QCompleter(namespaceList_, this);
  ...
  if (getParameterListClient_.call(req, res)) {
    ...
    if (res.parameters.size() == res.isIntegral.size()) {
      for (unsigned int i = 0; i < res.parameters.size(); i++) {
        parameterInfos_.emplace_back(res.parameters.at(i), res.isIntegral.at(i));
      }
...
void ParametersPlugin::drawParamList() {
  ...
  auto paramsScrollArea = new QScrollArea();
